...
AX_VOID COS08a20::DeInitEnhance() {
    SAMPLE_ENHANCE_DeInit();
}

AX_BOOL COS08a20::SnapshotProcess(AX_U8 nCapturePipeId, AX_U8 nChannel, AX_SNS_HDR_MODE_E eHdrMode, const AX_IMG_INFO_T** pImgInfo,
                                    AX_BOOL bDummy) {
// AX_BOOL Sample_CAM_CaptureFrameProc(AX_U32 nCapturePipeId, const AX_IMG_INFO_T *pImgInfo[]) {
    ...
    do {
        ...
        /* Step 2. 1st-frame YUV and Raw process */
        {
            ...
            axRet = AX_VIN_GetYuvFrame(nCapturePipeId, AX_VIN_CHN_ID_MAIN, &tYuv1stCaptureFrame, timeout);
            if (axRet != 0) {
            ...
            axRet = AX_VIN_ReleaseYuvFrame(nCapturePipeId, AX_VIN_CHN_ID_MAIN, &tYuv1stCaptureFrame);
            if (axRet != 0) {
                LOG_M_E(SENSOR, "1st AX_VIN_GetYuvFrame failed, ret=0x%x", axRet);
                break;
            }
