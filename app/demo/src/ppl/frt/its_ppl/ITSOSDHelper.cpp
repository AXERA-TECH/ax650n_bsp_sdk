...
AX_BOOL COSDHelper::StartOSD(CIVPSGrpStage* pIvpsInstance) {
    ...
    for (AX_U32 nChn = 0; nChn < pGrpConfig->nGrpChnNum; nChn++) {
        ...
        if (AX_TRUE != pGrpAttr->tPipelineAttr.tFilter[nChnFilter][APP_OSD_CHANNEL_FILTER].bEngage) {
            /* FIXME: As OSD applies on channel filter 1 now, check engine of this filter whether configured always. */
            LOG_MM_E(OSD, "[%d][%d] Engine not configured. Please check the engine_filter_3 configuration in ivps.json.", pGrpConfig->nGrp,
                     nChn);
            continue;
        }
        for (AX_U32 i = 0; i < vecOsdCfg.size(); i++) {
            ...
            if (AX_IVPS_INVALID_REGION_HANDLE != hRgn) {
                ...
                AX_S32 nFilter = ((nChn + 1) << 4) + APP_OSD_CHANNEL_FILTER;
                ...
                m_arrRgnThreadParam[nRgnIndex].nFilter = ((nChn + 1) << 4) + APP_OSD_CHANNEL_FILTER;
...
AX_VOID COSDHelper::TimeThreadFunc(OSD_REGION_PARAM_T* pThreadParam) {
    ...
    while (!pThreadParam->bExit) {
        ...
        AX_U32 nSrcWidth = m_pIvpsGrpInstance->GetGrpPPLAttr()->tPipelineAttr.tFilter[nIvpsChn + 1][APP_OSD_CHANNEL_FILTER].nDstPicWidth;
        AX_U32 nSrcHeight = m_pIvpsGrpInstance->GetGrpPPLAttr()->tPipelineAttr.tFilter[nIvpsChn + 1][APP_OSD_CHANNEL_FILTER].nDstPicHeight;
        AX_U32 nSrcOffset = 0;
        ...
        LOG_MM_D(OSD,
                 "[%d][%d] OSD (TIME): bEnable:%d,  nSrcWidth:%d, nSrcHeight:%d, u32BmpWidth: %d, u32BmpHeight: %d, xOffset: %d, yOffset: "
                 "%d, alpha: %d",
                 nIvpsGrp, nIvpsChn, tDisp.arrDisp[0].bShow, nSrcWidth, nSrcHeight, tDisp.arrDisp[0].uDisp.tOSD.u32BmpWidth,
    ...
    while (!pThreadParam->bExit) {
        ...
        if (APP_OSD_RECT(pThreadParam->hRgn, stRgn)) {
            ...
            for (; index < stRgn.size(); ++index) {
                if (index >= AX_IVPS_REGION_MAX_DISP_NUM) {
                    break;
                }
...
AX_VOID COSDHelper::UpdateOSDStr(OSD_REGION_PARAM_T* pThreadParam) {
    ...
    // AX_S32 nCharLen = swprintf(&wszOsdStr[0], MAX_OSD_WSTR_CHAR_LEN, L"%s", pThreadParam->tOsdCfg.tStrAttr.szStr);
...
AX_BOOL COSDHelper::Refresh() {
    IVPS_GROUP_CFG_T* pGrpConfig = m_pIvpsGrpInstance->GetGrpCfg();
    std::vector<OSD_CFG_T> vecOsdCfg;
    CWebOptionHelper::GetInstance()->GetOsdConfig(pGrpConfig->nSnsSrc, pGrpConfig->nGrp, 0, vecOsdCfg);

    for (AX_U32 i = 0; i < vecOsdCfg.size(); i++) {
        OSD_TYPE_E eType = vecOsdCfg[i].eType;
        for (AX_U32 j = 0; j < m_nRgnCount; j++) {
            if (m_arrRgnThreadParam[j].tOsdCfg.eType == eType && OSD_TYPE_RECT != eType) {
...
AX_BOOL COSDHelper::UpdateOSDRect(const std::vector<AX_APP_ALGO_BOX_T>& vecBox) {
    for (AX_U32 i = 0; i < m_nRgnCount; i++) {
        if (m_arrRgnThreadParam[i].tOsdCfg.eType == OSD_TYPE_RECT) {
            ...
            AX_U32 nSrcWidth =
                m_pIvpsGrpInstance->GetGrpPPLAttr()->tPipelineAttr.tFilter[nIvpsChn + 1][APP_OSD_CHANNEL_FILTER].nDstPicWidth;
            AX_U32 nSrcHeight =
                m_pIvpsGrpInstance->GetGrpPPLAttr()->tPipelineAttr.tFilter[nIvpsChn + 1][APP_OSD_CHANNEL_FILTER].nDstPicHeight;
