        ...
        if (!tRelation.bLink) {
            ...
            if (E_PPL_MOD_TYPE_AVS == tRelation.tSrcModChn.eModType) {
                ...
                for (AX_U8 j = 0; j < nOutChannels; j++) {
                    ...
                    if (-1 == pConfig->arrGrpFramerate[0]) {
                        pConfig->arrGrpFramerate[0] = m_mgrSensor.GetSnsInstance(nSnsID)
                                                          ->GetPipeAttr(tRelation.tSrcModChn.nGroup)
                                                          .tFrameRateCtrl.fDstFrameRate;
                    }
                    ...
                    if (-1 == pConfig->arrChnFramerate[j][0]) {
                        pConfig->arrChnFramerate[j][0] = m_mgrSensor.GetSnsInstance(nSnsID)
                                                             ->GetPipeAttr(tRelation.tSrcModChn.nGroup)
                                                             .tFrameRateCtrl.fDstFrameRate;
                    }
...
AX_BOOL CPanoBuilder::DispatchOpr(WEB_REQ_OPERATION_T& tOperation, AX_VOID** pResult /*= nullptr*/) {
    ...
    switch (eOperaType) {
        ...
        case E_WEB_OPERATION_TYPE_RESOLUTION: {
            ...
            PPL_MOD_INFO_T tDstMod = {E_PPL_MOD_TYPE_VENC, 0, tOperation.nChannel};
