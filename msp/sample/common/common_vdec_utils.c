...
#define AX_DEC_VALUE_2_STR_CASE(s32Ret) case (s32Ret): return(#s32Ret)
...
int h264_parse_sps(const uint8_t *buf, int len, SAMPLE_H264_SPS_DATA_T *sps)
{
    ...
    if (findSPS != AX_SUCCESS) {
        SAMPLE_LOG_ST("findSPS failed! Failed to parse width and height.\n"
...
FILE *OutputFileOpen(AX_CHAR **ppOutputFile, const SAMPLE_VDEC_OUTPUT_INFO_T *pInfo)
{
    ...
    if (*ppOutputFile == NULL) {
        ...
        *ppOutputFile = pFileOut;
        ...
        if (pInfo == NULL) {
            ...
            } else if (pInfo->u32CompressLevel) {
                ret = snprintf(pFileOut, AX_VDEC_FILE_NAME_LEN, "group%d_chn%d_format%d_compLevel%d_stride_%d_h_%d.lossy",
                               pInfo->VdGrp, pInfo->VdChn, pInfo->enImgFormat, pInfo->u32CompressLevel, pInfo->u32PicStride, pInfo->u32Height);
            } else {
                ret = snprintf(pFileOut, AX_VDEC_FILE_NAME_LEN, "group%d_chn%d_format%d_w_%d_h_%d.yuv",
                               pInfo->VdGrp, pInfo->VdChn, pInfo->enImgFormat, pInfo->u32Width, pInfo->u32Height);
            }
        }
    ...
    if (ret == 0) {
        ...
        if (slen >= AX_VDEC_FILE_NAME_LEN) {
            SAMPLE_CRIT_LOG("slen:%ld >= AX_VDEC_FILE_NAME_LEN:%d\n",
    ...
    if (fp_out == NULL) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, fopen pFileOut:%s FAILED!",
                       pInfo->VdGrp, pInfo->VdChn, pFileOut);
    }
...
static AX_S32 OutputSaveLossyFile(AX_VDEC_GRP VdGrp, AX_VDEC_CHN VdChn, const AX_VIDEO_FRAME_INFO_T *frameInfo,
                                  FILE *fp_out, AX_CHAR *pOutputFilePath)
{
    ...
    AX_S32 sRet = 0;
    AX_U32 tile_num;
    AX_U64 addr_offset, frame_size;
    AX_U64 buf_size;

    if (NULL == frameInfo || NULL == fp_out) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, NULL == frameInfo || NULL == fp_out\n", VdGrp, VdChn);
        s32Ret = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET;
    }

    if ((frameInfo->stVFrame.u64PhyAddr[0] == 0) || (frameInfo->stVFrame.u32PicStride[0] == 0)) {
        SAMPLE_LOG("VdGrp=%d, VdChn=%d, stVFrame.u64PhyAddr[0]:0x%llx stVFrame.u32PicStride[0]:%d\n",
                   VdGrp, VdChn, frameInfo->stVFrame.u64PhyAddr[0], frameInfo->stVFrame.u32PicStride[0]);
        s32Ret = 0;
        goto ERR_RET;
    }

    frame_size = frameInfo->stVFrame.u32PicStride[0] * frameInfo->stVFrame.u32Height;
    tile_num = frame_size / (AX_VDEC_LOSSY_YUV_LITE_HEIGHT * AX_VDEC_LOSSY_YUV_LITE_WIDTH);
    addr_offset = tile_num * AX_VDEC_LOSSY_COMP_LEVEL_FACTOR * frameInfo->stVFrame.stCompressInfo.u32CompressLevel;
    buf_size = addr_offset * 3 / 2;

    pLumaVirAddr = AX_SYS_Mmap(frameInfo->stVFrame.u64PhyAddr[0], buf_size);
    if (NULL == pLumaVirAddr) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AX_SYS_Mmap luma FAILED! .u64PhyAddr[0]:0x%llx\n",
                       VdGrp, VdChn, frameInfo->stVFrame.u64PhyAddr[0]);
        s32Ret = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET;
    }

    tmp_size = fwrite(pLumaVirAddr, 1, buf_size, fp_out);
    if (tmp_size != buf_size) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, fwrite FAILED! tmp_size:0x%x != FrameSize:0x%x",
                         VdGrp, VdChn, tmp_size, frameInfo->stVFrame.u32FrameSize);
        s32Ret = AX_ERR_VDEC_UNKNOWN;
    }

    if (pLumaVirAddr) {
        sRet = AX_SYS_Munmap(pLumaVirAddr, buf_size);
        if (sRet) {
            SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AX_SYS_Munmap luma FAILED, sRet=0x%x\n",
                           VdGrp, VdChn, sRet);
        }
    }
...
static AX_S32 OutputSaveYUVFile(AX_VDEC_GRP VdGrp, AX_VDEC_CHN VdChn, const AX_VIDEO_FRAME_INFO_T *frameInfo,
                                FILE *fp_out, AX_CHAR *pOutputFilePath)
{
    int ret = 0;
    AX_S32 sRet = 0;
    ...
    if (AX_FORMAT_YUV400 != pic_format) {
        ...
        if (NULL == pChromaVirAddr) {
            SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AX_SYS_Mmap luma FAILED! .u64PhyAddr[1]:0x%llx\n",
                       VdGrp, VdChn, frameInfo->stVFrame.u64PhyAddr[1]);
            s32Ret = AX_ERR_VDEC_UNKNOWN;
    ...
    SAMPLE_LOG("coded_width: %u, coded_height: %u, pic_stride: %u, \n"
               "coded_width_ch: %u, coded_h_ch: %u, pic_stride_ch: %u, pixel_bytes: %u, pic_format:%d\n",
               coded_width, coded_height, pic_stride,
               coded_width_ch, coded_h_ch, pic_stride_ch, 1, pic_format);
    ...
    for (i = 0; i < coded_height; i++) {
        ...
        if (tmp_size != coded_width) {
            SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, fwrite FAILED! tmp_size:0x%x != coded_width:0x%x",
                           VdGrp, VdChn, tmp_size, coded_width);
            s32Ret = AX_ERR_VDEC_UNKNOWN;
        }
        p_lu += pic_stride;
    }
    ...
    if (AX_FORMAT_YUV400 != pic_format) {
        ...
        for (i = 0; i < coded_h_ch; i++) {
            ...
            if (tmp_size != coded_width_ch) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, fwrite FAILED! tmp_size:0x%x != coded_width_ch:0x%x",
                               VdGrp, VdChn, tmp_size, coded_width_ch);
                s32Ret = AX_ERR_VDEC_UNKNOWN;
            }
            p_ch += pic_stride_ch;
        }
    ...
    ret = fflush(fp_out);
    if (ret) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, fflush FAILED! fp_out:%p",
                       VdGrp, VdChn, fp_out);
        s32Ret = AX_ERR_VDEC_UNKNOWN;
    }

    SAMPLE_LOG_N("VdGrp=%d, VdChn=%d, write YUV done! %s\n", VdGrp, VdChn, pOutputFilePath);

    if (pChromaVirAddr) {
        sRet = AX_SYS_Munmap(pChromaVirAddr, chromaMapSize);
        if (sRet) {
            SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AX_SYS_Munmap chroma FAILED, sRet=0x%x\n",
                           VdGrp, VdChn, sRet);
        }
    }
...
AX_S32 OutputFileSaveYUV(AX_VDEC_GRP VdGrp, AX_VDEC_CHN VdChn, const AX_VIDEO_FRAME_INFO_T *frameInfo, FILE *fp_out, AX_CHAR *pOutputFilePath)
{
    AX_S32 s32Ret = 0;

    if (NULL == frameInfo) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, NULL == frameInfo\n", VdGrp, VdChn);
        return -1;
    }

    if ((AX_COMPRESS_MODE_NONE != frameInfo->stVFrame.stCompressInfo.enCompressMode) &&
        (AX_COMPRESS_MODE_LOSSY != frameInfo->stVFrame.stCompressInfo.enCompressMode)) {
        SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, invalid enCompressMode: %d\n", VdGrp, VdChn,
                        frameInfo->stVFrame.stCompressInfo.enCompressMode);
        return -1;
    }

    if (AX_COMPRESS_MODE_LOSSY == frameInfo->stVFrame.stCompressInfo.enCompressMode)
        s32Ret = OutputSaveLossyFile(VdGrp, VdChn, frameInfo, fp_out, pOutputFilePath);
    else
        s32Ret = OutputSaveYUVFile(VdGrp, VdChn, frameInfo, fp_out, pOutputFilePath);
...
ret:
    ...
    if(0 == s32Ret) {
        ...
        if (pstStreamBuf->bRingbuf == AX_TRUE) {
            ...
            if (sReadLen != imgLen) {
                SAMPLE_CRIT_LOG("fread FAILED! sReadLen:0x%lx != imgLen:0x%x", sReadLen, imgLen);
                return -1;
            }
...
static AX_S32 __VdecUsrPicRead(FILE *pFileIn, AX_VIDEO_FRAME_T *pstFrame)
{
    ...
    pVaddr = (AX_VOID *)pstFrame->u64VirAddr[0];
    switch (eFmt) {
...
static AX_S32 __VdecUsrPicInfoFill(SAMPLE_VDEC_USRPIC_ARGS_T *pstUsrPicArgs, SAMPLE_VDEC_USERPIC_T *pstVdecUserPic)
{
    ...
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        ...
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            ...
            pstVdecUserPic->PoolId[VdChn] = AX_POOL_CreatePool(&stPoolConfig);
            if (AX_INVALID_POOLID == pstVdecUserPic->PoolId[VdChn]) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AX_POOL_CreatePool FAILED! BlkCnt:%d, BlkSize:0x%llx\n",
                                VdGrp, VdChn, stPoolConfig.BlkCnt, stPoolConfig.BlkSize);
                sRet = AX_ERR_VDEC_NOBUF;
                goto ERR_RET_DESTROY;
            }

            blkId = AX_POOL_GetBlock(pstVdecUserPic->PoolId[VdChn], FrameSize, NULL);
            if (AX_INVALID_BLOCKID == blkId) {
                SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, AX_POOL_GetBlock FAILED! PoolId:%d, BlkSize:0x%llx\n",
            ...
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64PhyAddr[0] = AX_POOL_Handle2PhysAddr(blkId);
            pstUserPic->stFrmInfo[VdChn].stVFrame.u64VirAddr[0] = (AX_U64)AX_POOL_GetBlockVirAddr(blkId);
    ...
    for (VdChn = 0; VdChn < AX_DEC_MAX_CHN_NUM; VdChn++) {
        ...
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            ...
            sRet = __VdecUsrPicRead(fpUserYUV, &pstUserPic->stFrmInfo[VdChn].stVFrame);
            if (sRet) {
    ...
    for (VdChn = 0; VdChn < tmp_ci; VdChn++) {
        ...
        if (pstUsrPicArgs->tChnParam[VdChn].bUserPicEnable) {
            if (pstVdecUserPic->BlkId[VdChn] != AX_INVALID_BLOCKID) {
                ret = AX_POOL_ReleaseBlock(pstVdecUserPic->BlkId[VdChn]);
                if (ret)
                    SAMPLE_CRIT_LOG("VdGrp=%d, VdChn=%d, PoolId[VdChn]:%d,  AX_POOL_ReleaseBlock FAILED! ret:0x%x %s",
                                    VdGrp, VdChn, pstVdecUserPic->BlkId[VdChn], ret, AX_VdecRetStr(ret));
            }
            if (pstVdecUserPic->PoolId[VdChn] != AX_INVALID_POOLID) {
                ret = AX_POOL_DestroyPool(pstVdecUserPic->PoolId[VdChn]);
                if (ret)
